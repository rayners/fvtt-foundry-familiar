...
describe('CollectionAnalyzer', () => {
  ...
  describe('listCollection', () => {
    ...
    it('should throw error for unknown collection type', () => {
      expect(() => analyzer.listCollection('unknown'))
        .toThrow('Unknown collection type: unknown');
    });
  });
  ...
  describe('getCollectionMember', () => {
    ...
    it('should throw error for non-existent member', () => {
      expect(() => analyzer.getCollectionMember('actors', 'nonexistent'))
        .toThrow('No actors found with ID: nonexistent');
    });

    it('should throw error for unknown collection type', () => {
      expect(() => analyzer.getCollectionMember('unknown', 'id'))
        .toThrow('Unknown collection type: unknown');
    });
  });
