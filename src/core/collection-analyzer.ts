...
export class CollectionAnalyzer {
  ...
  listCollection(type: string): Array<{
    ...
    if (!collection) {
      throw new Error(
        `Unknown collection type: ${type}. Available types: ${this.getAvailableCollectionTypes().join(', ')}`
      );
    }
  ...
  private extractPlaylistContent(entry: any): string {
    ...
    if (entry.sounds?.size > 0) {
      ...
      const soundList = Array.from(entry.sounds)
  ...
  private extractTableContent(entry: any): string {
    ...
    if (entry.results?.size > 0) {
      ...
      const resultList = Array.from(entry.results)
        .slice(0, 3)
        .map((result: any) => `${result.range}: ${result.text}`);
  ...
  private extractMacroContent(entry: any): string {
    ...
    if (entry.command) {
      const command =
        entry.command.length > 200 ? entry.command.substring(0, 200) + '...' : entry.command;
      parts.push(`Command:\n${command}`);
    }
